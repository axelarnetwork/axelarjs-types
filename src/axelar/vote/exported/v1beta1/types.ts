...
export const Vote = {
  $type: "axelar.vote.exported.v1beta1.Vote" as const,
  ...
  fromJSON(object: any): Vote {
    return {
      $type: Vote.$type,
      resultsDeprecated: gt.Array.isArray(object?.resultsDeprecated)
  ...
  toJSON(message: Vote): unknown {
    const obj: any = {};
    if (message.resultsDeprecated?.length) {
      obj.resultsDeprecated = message.resultsDeprecated.map((e) => Any.toJSON(e));
    }
    if (message.result !== undefined) {
      obj.result = Any.toJSON(message.result);
    }
